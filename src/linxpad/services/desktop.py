...
class DesktopScanner:
    ...
    def scan(self) -> list[dict]:
        ...
        return self._scan_dirs(_DESKTOP_DIRS)

    def _scan_dirs(self, dirs: list) -> list[dict]:
        found, seen = [], set()
        for d in dirs:
    ...
    def _parse(self, filepath: str) -> dict | None:
        try:
            with open(filepath, encoding="utf-8", errors="ignore") as f:
                content = f.read()
            return self._parse_content(content, os.path.basename(filepath))
        except Exception:
            logger.debug("Failed to parse %s", filepath, exc_info=True)
        return None

    def _parse_content(self, content: str, filename: str) -> dict | None:
        try:
            ...
            logger.debug("Failed to parse content for %s", filename, exc_info=True)
        return None
